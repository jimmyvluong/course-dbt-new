...
-- ALTERNATE OVERALL_CONVERSION RATE
...
--   --, stg_greenery__events.product_id
--   --, stg_greenery__products.product_name
--   , MAX(CASE WHEN stg_greenery__events.event_type = 'add_to_cart' then 1.0 else 0 end) AS add_to_cart
--   , MAX(CASE WHEN stg_greenery__events.event_type = 'checkout' then 1.0 else 0 end) AS checkout
--   , MAX(CASE WHEN stg_greenery__events.event_type = 'page_view' then 1.0 else 0 end) AS page_view
--   , MAX(CASE WHEN stg_greenery__events.event_type = 'package_shipped' then 1.0 else 0 end) AS package_shipped
-- FROM dbt_jimmy_l.stg_greenery__events
-- GROUP BY 1
-- )
-- SELECT
-- SUM(checkout)/COUNT(*) AS overall_conversion_rate
-- FROM session_counts;